...
'3) 중복 로그인 : @login 2
'4) 또 로그인?? : @login 3
Function TA_Login(UserNumber As Integer, MSG As String)
    Dim IN_ID As String
...
Function TA_FindID(UserNumber As Integer, MSG As String)
...
Function TA_FindPass(UserNumber As Integer, MSG As String)
    Dim IN_ID As String
    Dim IN_MAIL As String
    Dim i As Integer
...
'2) 계정 중복 : @join 1
'3) 로그인상태: @join 2
Function TA_Join(UserNumber As Integer, MSG As String)
    Dim IN_ID As String
    Dim IN_PASS As String
    Dim IN_NAME As String
    Dim IN_MAIL As String
...
'2) 입력 아이디 존재 : @dupid 1
Function TA_DupID(UserNumber As Integer, MSG As String)
    Dim IN_ID As String
    Dim i As Integer
...
Function TA_Logout(UserNumber As Integer, MSG As String)
...
Function TA_Record(UserNumber As Integer, MSG As String)
    ...
    On Error GoTo Err
    RecordRecord(IN_GAME).MoveFirst
    For i = 0 To RecordRecord(IN_GAME).RecordCount - 1
        If RecordRecord(IN_GAME).Fields(1) = iUser(UserNumber).NAME Then  '자신의 아이디 찾으면
            FoundRecord = FoundRecord & Format(RecordRecord(IN_GAME).Fields(2), "yy-mm-dd") & "," & RecordRecord(IN_GAME).Fields(4) & "/"
        End If
    RecordRecord(IN_GAME).MoveNext
    Next i
    
    If FoundRecord = "" Then
        A_Record = "@record 1" '"기록이 없습니다."
    Else
        A_Record = "@record 0," & FoundRecord
    End If
...
'2) 오늘 기록 없음 : @today 1
'3) 로그인 하지 않음 : @today 2
Function TA_Today(UserNumber As Integer, MSG As String)
    Dim IN_GN As String
    Dim i As Integer
...
'1) 점수 저장 성공 : @save 0
'2) 로그인 하지 않음 : @save 1
Function TA_Save(UserNumber As Integer, MSG As String)
    Dim IN_GN As String
...
Function TA_Average(UserNumber As Integer, MSG As String)
...
Function TA_GetInfo(UserNumber As Integer, MSG As String)
    'Dim IN_TEMP As String
    Dim FoundRecord As String
    Dim i As Integer
...
Function TA_SetInfo(UserNumber As Integer, MSG As String)
    Dim IN_ID As String
...
'2) 로그인 하지 않음: @leave 1
'3) 비밀번호 틀림 : @leave 2
Function TA_Leave(UserNumber As Integer, MSG As String)
    Dim IN_ID As String
